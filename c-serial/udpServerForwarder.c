...
static void handle_range_profile(const uint8_t *payload, uint32_t length) {
    ...
    // SAFE FOR ARM: Copy unaligned payload into an aligned stack array instead of direct casting
...
static void handle_range_doppler_heatmap(const uint8_t *payload, uint32_t length) {
    ...
    // SAFE FOR ARM: Use stack space to mirror the matrix elements safely aligned
...
static void process_radar_frame(const uint8_t *frame_data, size_t size) {
    ...
    for (uint32_t i = 0; i < header.numTLVs; i++) {
        ...
        // TI mmWave SDK aligns the next TLV structure to a 4-byte boundary
        if (offset % 4 != 0) {
            offset += (4 - (offset % 4));
        }
    }
...
static void port2_feed(char *accum, size_t *accum_len, const char *chunk, size_t n) {
    ...
    *accum_len += n;
    ...
    while (*accum_len >= sizeof(RadarFrameHeader)) {
        ...
        // SAFE FOR ARM: Extract uint32_t using memcpy to prevent unaligned read trap
...
int main() {
    ...
    // SAFE FOR ARM: Force 16-byte buffer alignment to prevent memory faults on Raspberry Pi
