...
class KickToOpen(Plot2D):
    ...
    def setupGUI(self, gridLayout, demoTabs, device):
        ...
        self.ktoPaneLayout = QGridLayout()
        ...
        # Initialize the model pane and layout
        modelPane = QGroupBox("Model")
        modelPaneLayout = QVBoxLayout()

        # Setup model status box
        ktoTempDict['modelStatus'] = QLabel("Present Model")
        ktoTempDict['modelStatus'].setAlignment(Qt.AlignCenter)
        ktoTempDict['modelStatus'].setStyleSheet('background-color: lightgray; color: white; font-size: 40px; font-weight:bold')
        modelPaneLayout.addWidget(ktoTempDict['modelStatus'])
        modelPane.setLayout(modelPaneLayout)
        ktoTempDict['modelPane'] = modelPane
        ...
        # Put the widgets into the layout
        ktoPlotPaneLayout.addWidget(ktoTempDict['presenceNote'], 0, 0,1,1, alignment=Qt.AlignCenter)
        ktoPlotPaneLayout.addWidget(ktoTempDict['presenceplot'], 1,0,1,1)
        ktoPlotPaneLayout.addWidget(ktoTempDict['dopplerplot'], 1,1,1,1)
        ...
        self.ktoPaneLayout.addWidget(powerPane, 3,0,1,2)
        self.ktoPaneLayout.addWidget(modelPane, 2, 0, 1, 2)
        self.ktoPaneLayout.addWidget(ktoStatusPane, 1,0,1,2)
        self.ktoPaneLayout.addWidget(ktoPlotPane, 0,0,1,2)

        self.ktoPane.setLayout(self.ktoPaneLayout)
        ...
        # Change 2D Plot View
        self.scatterPlot2D.setXRange(0,1,padding=0.01)
        self.scatterPlot2D.setYRange(-1,1,padding=0.01)
        self.scatterPlot2D.showGrid(x=True,y=True)
    ...
    def updateGraph(self, outputDict):
        ...
        if ('pointCloud' in outputDict):
            self.scatter.setData(pos=outputDict['pointCloud'][:][0:2])

        # Add this section to handle the model flag
        if ('modelFlag' in outputDict):
            modelNum = outputDict['modelFlag']
            self.kto['modelStatus'].setText(f"Model: {modelNum}")

            colors = {0: 'gray', 1: 'lightblue', 2: 'blue'}
            color = colors.get(modelNum, 'purple')  
            self.kto['modelStatus'].setStyleSheet(f'background-color: {color}; color: white; font-size: 40px; font-weight:bold')
        ...
        if all(key in outputDict for key in ['ktoGesture', 'gestureFeatures', 'localization_mag', 'localization_range', 'localization_elevidx', 'localization_azimidx']):
            ...
            self.kto['presenceNote'].setVisible(False)
            ...
            self.gestureMode = KTO_GESTURE_MODE

            if (self.ktoGesture is KTO_KICK):
                self.updateKTOGestureDisplay('Gesture Detected')
    ...
    def parseSOSOCfg(self, args):
        self.xMinThresholdBin = float(args[9])
        self.xMaxThresholdBin = float(args[10])
        self.yMinThresholdBin = float(args[11])
        self.yMaxThresholdBin = float(args[12])

        self.frontLine = pg.InfiniteLine(pos=self.xMinThresholdBin, angle=90, movable=False, pen=pg.mkPen('g', width=1, style=Qt.SolidLine))
        self.backLine = pg.InfiniteLine(pos=self.xMaxThresholdBin, angle=90, movable=False, pen=pg.mkPen('g', width=1, style=Qt.SolidLine))
        self.leftLine = pg.InfiniteLine(pos=self.yMinThresholdBin, angle=0, movable=False, pen=pg.mkPen('g', width=1, style=Qt.SolidLine))
        self.rightLine = pg.InfiniteLine(pos=self.yMaxThresholdBin, angle=0, movable=False, pen=pg.mkPen('g', width=1, style=Qt.SolidLine))
        
        # Only add the 
        self.ktoPaneLayout.addWidget(self.plot_2d, 0, 2, 2, 2)

        self.scatterPlot2D.addItem(self.frontLine)
        self.scatterPlot2D.addItem(self.backLine)
        self.scatterPlot2D.addItem(self.leftLine)
        self.scatterPlot2D.addItem(self.rightLine)
