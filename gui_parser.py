...
class UARTParser():
    ...
    def readAndParseUartDoubleCOMPort(self, demo):
        ...
        if (self.parserType == "DoubleCOMPort"):
            outputDict = parseStandardFrame(frameData, self.demo)
        else:
    ...
    def connectComPorts(self, cliCom, dataCom, cliBaud, dataBaud):
        self.cliCom = serial.Serial(cliCom, cliBaud, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=0.6)
        self.dataCom = serial.Serial(dataCom, dataBaud, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=0.6)
