...
from PySide6.QtWidgets import QGroupBox, QGridLayout, QLabel, QWidget, QHBoxLayout, QVBoxLayout, QTabWidget
...
class GestureRecognition():
    ...
    def gesturePostProc(self, ann_probs):
        ...
        if self.prevFramegesture != self.currFramegesture:
            if self.currFramegesture != self.gestureList.index(GESTURE_NONE):
                self.gestureHandler(self.currFramegesture)
        else:
            if self.frameNum % self.contGestureFramecount == 0 :
                if self.currFramegesture == self.gestureList.index(GESTURE_TWIRL_CW) or self.currFramegesture == self.gestureList.index(GESTURE_TWIRL_CCW) or self.currFramegesture == self.gestureList.index(GESTURE_SHINE):
                    self.gestureHandler(self.currFramegesture) 
