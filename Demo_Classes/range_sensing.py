...
from PySide6.QtWidgets import QGroupBox, QHBoxLayout, QLabel, QWidget, QVBoxLayout

# Local Imports
from Common_Tabs.plot_1d import Plot1D

# Logger
class RangeSensing(Plot1D):
    def __init__(self):
    ...
    def setupGUI(self, gridLayout, demoTabs, device):
        ...
        self.levelsensingTab.setLayout(vboxLevelSense)
        demoTabs.addTab(self.levelsensingTab, '1D Sensing')
        ...
        if (pointCloud is not None and numPoints is not None):
            for i in range(numPoints):
                if(i == 0):
                    ...
                    self.Peak1Magnitude =  round(20 * np.log10(round(pointCloud[i, 4], 4)), 2)
                elif (i == 1):
                    self.Peak2 = round(pointCloud[i, 1], 3)
                    self.Peak2Magnitude =  round(20 * np.log10(round(pointCloud[i, 4], 4)), 2)
                elif (i == 2):
                    self.Peak3 = round(pointCloud[i, 1], 3)
                    self.Peak3Magnitude =  round(20 * np.log10(round(pointCloud[i, 4], 4)), 2)
    ...
    def parseRangeSensingCfg(self, args):
        self.NumOfAdcSamples = int(args[8])
